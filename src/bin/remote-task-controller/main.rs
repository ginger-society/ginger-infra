...
async fn reconcile(
    ...
    match phase {
        ...
        RemoteTaskPhase::Running => {
            println!(
                "[tekton-controller] {}/{} already Running — no resume logic, leaving as-is",
                ns, name
            );
        }
        RemoteTaskPhase::Succeeded | RemoteTaskPhase::Failed => {
            println!(
                "[tekton-controller] {}/{} terminal, nothing to do",
                ns, name
            );
        }
    }
    ...
    for env in env_specs {
        ...
        if let Some(from) = &env.value_from {
            if let Some(secret_ref) = &from.secret_key_ref {
                ...
                let data = secret
                    .data
                    .ok_or_else(|| ReconcileError::SecretKeyNotFound {
...
async fn stream_job(
    ...
    let response = http
        ...
        .await
        .map_err(|e| {
            ReconcileError::SidekickRequest(format!("request to '{url}' failed: {e}"))
        })?;
    ...
    while let Some(chunk) = stream.next().await {
        let chunk = chunk.map_err(|e| {
            ReconcileError::SidekickRequest(format!("stream read error: {e}"))
        })?;
